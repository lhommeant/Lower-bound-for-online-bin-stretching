...
int HASH_TABLE_SIZE = 100000000; // Size of the hash table containing configurations
...
// --- KNOWING WHICH ITEM CAN BE SEND  ---
...
void insert_hash_table(uint64_t key, int *bin_config, int *instance, struct hash_table_element* hash_table, int winning) {
    ...
    write_hash_table(key, bin_config, instance, winning, real_key, hash_table);
    HASH_FAILS++;
}
 ...
 */
int algorithm_to_play(int nb_objects_given,
                        int total_weight,
                        int new_object,
    ...
    for (int bin=0; bin<NB_BINS; bin++) {
        ...
        if (nb_objects_given == 1) {
            STATUS2 = bin;
            STATUS3 = 0;
        }
        if (nb_objects_given == 2) {
            STATUS3 = bin;
        }
        if (packings[bin]+new_object<TARGET) {
            // We can fit the object in this bin without reaching TARGET
                ...
                if (hash_table_result<=-1) {
                    ...
                    if (hash_table_result == -2) { // Configuration is a green packing
                        ...
                        if (res) {
                            ...
                            insert_hash_table(new_hash, packings, objects, hash_table, 1);
                            // We remove the item from the bin
                            while(switch_bin<bin) {
                                packings[switch_bin] = packings[switch_bin+1];
                                switch_bin++;
 ...
 */
int adversary_to_play(int nb_objects_given,
                        int total_weight,
                        int previous_maximal_weight,
    ...
    if (NODES_VISITED%NODES_BETWEEN_PRINTS == 0) {
        printf("----------------------------------------------------------------------------------\n");
        printf("Progress: %i - %i - %i / %i\n", STATUS, STATUS2, STATUS3, NB_BINS);
    ...
    #endif
            int res = algorithm_to_play(nb_objects_given, total_weight, item, max_value, new_nb_reachable_packings, hash, objects, object_seq, packings, hash_table);
            if (res) {
                return res;
            }
        }
    ...
    return 0; // there is an algorithm that can fit items under TARGET
}
...
void compute_bound_feasibility() {
    ...
    int res = adversary_to_play(0, 0, CAP, 0, starting_hash, objects, object_sequence, packings, hash_table);
    if (res) {
        printf("\n   --------------------------------------\n");
...
{   
    ...
    for (int arg = 1; arg<argc; arg++) {
        if (strcmp(argv[arg], "-h") == 0) {
            ...
            printf("-draw to allow the program to draw .svg files representing algorithm winning packings for 2 or 3 bins\n");
            return 1;
        }
